...
$ob->getFundPerformace(function ($index) {
    ...
    if($lastIndex !== null){
        echo "=== Ongoing ".date("Y-m F", $index[$lastIndex]["timestamp"]).": Sum of values ".number_format($currentMonth, 3)."% (".number_format($realMonth, 3)."%) / Compounded growth ".number_format($currentCompound * 100, 3)."% ===\n\n";
        echo "=== TOTAL: Sum of values ".number_format($totalYear, 3)."% / Compounded growth ".number_format($totalCompound * 100, 3)."% ===\n\n";
    }
